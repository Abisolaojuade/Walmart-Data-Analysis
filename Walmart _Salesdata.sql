...
Select count(distinct City) from Walmart_Salesdata;
...
Select count(distinct Product_line) from Walmart_salesdata;
...
group by Product_Line;
 Select avg(cogs) from walmart_salesdata;
...
Select Product_line, Round(Avg(rating),2) from Walmart_salesdata
Group by Product_line;
...
Select count(distinct Customer_Type) from Walmart_Salesdata;

-- 2. How many unique payment methods does the data have?
Select count(Distinct Payment) from Walmart_salesdata;
